...
class DatabaseManager:
    ...
    def _initialize_knowledge_base_collection(self) -> None:
        ...
        try:
            ...
            logger.warning(f"Vector search index creation failed (may not be supported): {e}")
   ...
   # Vector Database Operations
        ...
        try:
            ...
            if documents:
                try:
                    ...
                    logger.info(f"Successfully stored {len(result.inserted_ids)} embeddings")
                    return True
                except DuplicateKeyError as e:
                    # Handle duplicate keys by updating existing documents
                    logger.warning(f"Duplicate keys found, updating existing documents: {e}")
    ...
    def search_similar(self, query_embedding: List[float], limit: int = 5, 
                      file_type_filter: Optional[str] = None) -> List[SearchResult]:
        """
        ...
        try:
            ...
            try:
                return self._vector_search(collection, query_embedding, limit, file_type_filter)
            except Exception as e:
                logger.warning(f"Vector search failed, falling back to cosine similarity: {e}")
                return self._cosine_similarity_search(collection, query_embedding, limit, file_type_filter)
    ...
    def _vector_search(self, collection: Collection, query_embedding: List[float], 
                      limit: int, file_type_filter: Optional[str]) -> List[SearchResult]:
    ...
    def _cosine_similarity_search(self, collection: Collection, query_embedding: List[float], 
                                 limit: int, file_type_filter: Optional[str]) -> List[SearchResult]:
    ...
    def delete_vectors_by_path(self, document_path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Deleted {result.deleted_count} vectors for path: {document_path}")
            return True
            ...
            logger.error(f"Unexpected error deleting vectors for path {document_path}: {e}")
            return False
...
def create_embedded_chunk(chunk_id: str, document_path: str, content: str, 
                         embedding: List[float], chunk_index: int, 
                         file_type: str, metadata: Optional[Dict[str, Any]] = None) -> EmbeddedChunk:
    """
...
def validate_embedding_dimensions(embeddings: List[List[float]], expected_dim: int) -> bool:
    """
