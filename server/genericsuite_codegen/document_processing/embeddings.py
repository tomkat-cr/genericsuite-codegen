...
class OpenAIEmbeddingProvider(EmbeddingProvider):
    ...
    def __init__(self,
        ...
        """
        if openai is None:
            raise ImportError("OpenAI library not installed. Install with: "
                              "pip install openai")
    ...
    def __init__(self,
        ...
        if device is None:
            if torch and torch.cuda.is_available():
                device = 'cuda'
            elif torch and hasattr(torch.backends, 'mps') and \
...
class EmbeddingGenerator:
    ...
    def generate_embeddings_for_chunks(self, chunks) -> List[EmbeddedChunk]:
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            try:
                ...
                batch_embeddings = self.provider.generate_embeddings(
                    batch_texts)
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        #     'max_tokens': getattr(self.provider, 'model_config', {}).get(
        #         'max_tokens', 'unknown')
        # }
...
async def generate_embeddings_async(
    chunks, provider: Optional[str] = None,
    model: Optional[str] = None
) -> List[EmbeddedChunk]:
