        ...
        for sentence in sentences:
            ...
            if len(potential_chunk) <= self.max_chunk_size:
                ...
                if len(sentence) <= self.max_chunk_size:
                    ...
                    sentence_chunks = fixed_chunker.chunk(sentence, document_metadata)
        ...
        for paragraph in paragraphs:
            ...
            if len(potential_chunk) <= self.max_chunk_size:
                ...
                if len(paragraph) <= self.max_chunk_size:
                    ...
                    sentence_chunker = SentenceChunker(self.max_chunk_size, self.min_chunk_size)
                    paragraph_chunks = sentence_chunker.chunk(paragraph, document_metadata)
...
class CodeChunker(ChunkingStrategy):
    ...
    def chunk(self, text: str, document_metadata: Dict[str, Any]) -> List[str]:
        ...
        if file_type == 'python':
            ...
            paragraph_chunker = ParagraphChunker(self.max_chunk_size, self.min_chunk_size)
    ...
    def _chunk_js_code(self, text: str) -> List[str]:
        ...
        function_pattern = re.compile(r'^\s*(function\s+\w+|const\s+\w+\s*=|let\s+\w+\s*=|var\s+\w+\s*=|\w+\s*:\s*function|export\s+function)', re.MULTILINE)
...
class DocumentChunker:
    ...
    def chunk_document(self, document) -> List[DocumentChunk]:
        ...
        logger.debug(f"Chunked document {document.path} into {len(document_chunks)} chunks")
        return document_chunks
    
    def get_optimal_chunk_size(self, text: str, target_embedding_model: str = "default") -> int:
        """
        Calculate optimal chunk size based on text characteristics and embedding model.
        ...
        if "openai" in target_embedding_model.lower():
            # OpenAI models work well with chunks up to 8192 tokens (~6000 chars)
            max_size = 6000
        elif "sentence-transformers" in target_embedding_model.lower():
            # Sentence transformers typically have 512 token limits (~400 chars)
            max_size = 400
        else:
...
def chunk_document(document, strategy: str = "adaptive", **kwargs) -> List[DocumentChunk]:
    """
    Convenience function to chunk a document.
    
    Args:
        document: Document to chunk
        strategy: Chunking strategy ("adaptive", "fixed", "sentence", "paragraph", "code")
...
def get_chunking_stats(chunks: List[DocumentChunk]) -> Dict[str, Any]:
    ...
    return {
        ...
        'strategies_used': list(set(chunk.metadata.get('chunking_strategy', 'unknown') 
                                  for chunk in chunks))
    }