...
class RepositoryCloner:
    ...
    def clone_repository(self, repo_url: str, force_refresh: bool = True) -> bool:
        """
        ...
        try:
            ...
            if self.local_dir.exists():
                if force_refresh:
                    logger.info(f"Removing existing directory: {self.local_dir}")
                    shutil.rmtree(self.local_dir)
                else:
                    # Try to update existing repository
                    try:
                        repo = git.Repo(self.local_dir)
                        logger.info(f"Updating existing repository: {self.local_dir}")
                        repo.remotes.origin.pull()
                        return True
                    except Exception as e:
                        logger.warning(f"Could not update existing repository: {e}")
...
class DocumentIngestionOrchestrator:
    ...
    def process_files(self) -> List[Document]:
        ...
        try:
            ...
            logger.info(f"Processed {len(documents)} documents from {len(files_to_process)} files")
            return documents
    ...
    def chunk_documents(self, documents: List[Document]) -> List[DocumentChunk]:
        ...
        try:
            ...
            for i, document in enumerate(documents):
                ...
                try:
                    chunks = chunk_document(document, strategy=self.chunking_strategy)
                    all_chunks.extend(chunks)
                except Exception as e:
                    logger.warning(f"Error chunking document {document.path}: {e}")
    ...
    def store_vectors(self, embedded_chunks: List[EmbeddedChunk]) -> bool:
        ...
        try:
            ...
            if success:
                self._update_progress(increment_completed=True)
                logger.info(f"Stored {len(embedded_chunks)} vectors in database")
            else:
                self._update_progress(
                    status=IngestionStatus.FAILED,
    ...
    def run_full_ingestion(self, force_refresh: bool = True) -> Dict[str, Any]:
        ...
        Args:
            force_refresh: If True, clean up existing data and re-process everything
        ...
        try:
            ...
            if force_refresh:
                if not self.cleanup_existing_vectors():
                    return self._get_failure_result("Failed to clean up existing vectors")
    ...
    def _get_success_result(self, 
                           ...
                           embedded_chunks: List[EmbeddedChunk]) -> Dict[str, Any]:
        ...
        return {
            ...
            'statistics': {
                ...
                'embedding_model': self.embedding_generator.get_model_info() if self.embedding_generator else None,
