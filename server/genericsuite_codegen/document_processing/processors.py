...
class FileFilter:
    ...
    INCLUDED_EXTENSIONS = {
        ...
        '.ini', '.nvmrc', '.npmrc', '.python-version', '.toml', '.yaml', '.yml',
    ...
    def _matches_gitignore_pattern(self, file_path: Path, pattern: str) -> bool:
        ...
        if pattern.endswith('/'):
            pattern = pattern[:-1]
            return file_path.is_dir() and re.match(pattern, str(file_path.name))
    ...
    def should_include_file(self, file_path: Path) -> bool:
        ...
        if extension not in self.INCLUDED_EXTENSIONS:
            ...
            if not extension and file_path.name in {'.babelrc', '.dockerignore', '.gitignore'}:
        ...
        for part in parts:
            if part.startswith('.') and part not in {'.env', '.gitignore', '.dockerignore', '.babelrc'}:
...
class BaseProcessor:
    ...
    def _create_document(self, file_path: Path, content: str, file_type: str) -> Document:
...
class DocumentProcessorManager:
    ...
    def process_repository(self) -> List[Document]:
        ...
        logger.info(f"Processing {len(files_to_process)} files from {self.repo_path}")
