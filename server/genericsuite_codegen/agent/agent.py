...
class AgentConfig(BaseModel):
    ...
    model_provider: str = Field(default="openai", description="LLM provider (openai, litellm)")
    model_name: str = Field(default="gpt-4", description="Model name to use")
    temperature: float = Field(default=0.1, description="Model temperature (0.0 to 1.0)")
...
class QueryRequest(BaseModel):
    ...
    task_type: str = Field(default="general", description="Type of task (general, json, python, frontend, backend)")
    framework: Optional[str] = Field(default=None, description="Backend framework for code generation")
    context_limit: int = Field(default=4000, description="Maximum context length")
    include_sources: bool = Field(default=True, description="Include source attribution in response")
...
class GenericSuiteAgent:
    ...
    def __init__(self, config: Optional[AgentConfig] = None):
        ...
        logger.info(f"Initialized GenericSuite agent with {self.config.model_provider} provider")
    ...
    def _initialize_model(self) -> Model:
        ...
        try:
            if self.config.model_provider == "openai":
                ...
                logger.warning(f"Unsupported provider {self.config.model_provider}, falling back to OpenAI")
                return self._create_openai_model()
    ...
    async def query(self, request: QueryRequest, 
        ...
        try:
            ...
            logger.info(f"Processing query: '{request.query}' (type: {request.task_type})")
            ...
            # Execute query
            result = await self.agent.run(prompt, message_history=run_context.get("history", []))
            ...
            logger.info(f"Generated response ({len(response.content)} chars) with {len(sources)} sources")
            return response
    ...
    def _create_task_prompt(self, request: QueryRequest, context: str, 
        ...
        if request.task_type in ["json", "python", "frontend", "backend"]:
            ...
            if request.task_type == "backend" and request.framework:
                framework_prompt = self.prompt_manager.get_framework_specific_prompt(request.framework)
                context += f"\n\n{framework_prompt}"
    ...
    def _create_run_context(self, context: Optional[AgentContext], 
    ...
    def _format_response(self, result: Any, request: QueryRequest, 
                        sources: List[str], context: str) -> AgentResponse:
        """
    ...
    async def generate_json_config(self, requirements: str, 
                                 config_type: str = "table") -> AgentResponse:
        """
    ...
    async def generate_python_code(self, requirements: str, 
                                 code_type: str = "tool") -> AgentResponse:
        """
    ...
    async def generate_backend_code(self, requirements: str, 
                                  framework: str = "fastapi") -> AgentResponse:
        """
...
def create_agent_config_from_env() -> AgentConfig:
    ...
    return AgentConfig(
        ...
        max_tokens=int(os.getenv("GENERICSUITE_LLM_MAX_TOKENS", "4000")) if os.getenv("GENERICSUITE_LLM_MAX_TOKENS") else None,
