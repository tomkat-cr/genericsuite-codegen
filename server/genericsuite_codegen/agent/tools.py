...
class KnowledgeBaseQuery(BaseModel):
    ...
    file_type_filter: Optional[str] = Field(default=None, description="Optional file type filter (e.g., 'py', 'md', 'json')")
...
class KnowledgeBaseSearchResults(BaseModel):
    ...
    context_summary: str = Field(description="Summary of the retrieved context")
    sources: List[str] = Field(description="List of unique source document paths")
...
class KnowledgeBaseTool:
    """
    Knowledge base search tool for vector similarity search and context retrieval.
    ...
    def search(self, query: str, limit: int = 5, 
               file_type_filter: Optional[str] = None) -> KnowledgeBaseSearchResults:
        """
        ...
        Returns:
            KnowledgeBaseSearchResults: Search results with context and attribution.
        ...
        """
        try:
            logger.info(f"Searching knowledge base for query: '{query}' (limit: {limit})")
            ...
            for result in search_results:
                ...
                file_type = result.metadata.get('file_type', 
                    result.document_path.split('.')[-1] if '.' in result.document_path else 'unknown')
            ...
            context_summary = self._generate_context_summary(search_results, query)
            ...
            logger.info(f"Found {len(result_models)} results from {len(sources)} sources")
    ...
    def _generate_context_summary(self, results: List[SearchResult], query: str) -> str:
        """
        ...
        if len(sources) > 1:
            summary_parts.append(f"Information sourced from {len(sources)} different documents.")
        
        if file_types:
            file_type_desc = ", ".join([f"{count} {ftype}" for ftype, count in file_types.items()])
    ...
    def rank_context_by_relevance(self, results: List[SearchResult], 
                                 query: str) -> List[ContextRanking]:
        """
    ...
    def _calculate_relevance_score(self, result: SearchResult, query_lower: str) -> float:
        """
        Calculate enhanced relevance score combining similarity and text matching.
        ...
        word_bonus = (word_matches / len(query_words)) * 0.05 if query_words else 0.0
        ...
        final_score = base_score + phrase_bonus + word_bonus + file_type_bonus + length_penalty
        
        # Ensure score stays within reasonable bounds
        return max(0.0, min(1.0, final_score))
    
    def get_context_for_generation(self, query: str, max_context_length: int = 4000,
                                  file_type_filter: Optional[str] = None) -> Tuple[str, List[str]]:
        """
        ...
        Returns:
            Tuple[str, List[str]]: Formatted context string and list of sources.
        """
        try:
            ...
            for context in ranked_context:
                ...
                if current_length + len(content_with_source) > max_context_length:
                    ...
                    if remaining_space > 100:  # Only add if we have reasonable space
                        truncated_content = context.content[:remaining_space] + "..."
            ...
            return final_context, list(set(sources))  # Remove duplicate sources
...
class JSONConfigRequest(BaseModel):
    ...
    config_type: str = Field(description="Type of configuration (table, form, menu, auth)")
    requirements: str = Field(description="Requirements and specifications for the configuration")
    table_name: Optional[str] = Field(default=None, description="Name of the table (for table configs)")
    include_validation: bool = Field(default=True, description="Include validation rules")
    include_examples: bool = Field(default=True, description="Include example values and comments")


class JSONConfigResult(BaseModel):
    """Result model for JSON configuration generation."""
    configuration: Dict[str, Any] = Field(description="Generated JSON configuration")
    config_type: str = Field(description="Type of configuration generated")
    validation_notes: List[str] = Field(default_factory=list, description="Validation and usage notes")
    examples: Dict[str, Any] = Field(default_factory=dict, description="Example configurations")
...
class JSONConfigGenerator:
    ...
    def generate_table_config(self, requirements: str, table_name: str,
                            include_validation: bool = True) -> JSONConfigResult:
        """
        ...
        try:
            ...
            if include_validation:
                config["table_config"]["validations"] = self._generate_validation_rules(fields)
            
            # Configure UI settings
            config["table_config"]["ui_config"] = self._generate_ui_config(fields, table_name)
            
            # Generate validation notes
            validation_notes = self._generate_validation_notes(config, requirements)
    ...
    def generate_form_config(self, requirements: str, form_name: str) -> JSONConfigResult:
        """
        ...
        try:
            ...
            config["form_config"]["validation_rules"] = self._generate_form_validation_rules(fields)
            
            # Configure UI layout
            config["form_config"]["ui_layout"] = self._generate_form_layout(fields, form_name)
        ...
        for field_name, field_type in field_patterns.items():
            if field_name in requirements_lower:
                ...
                if field_name in ["name", "title"]:
                    ...
                    field_config["options"] = self._get_default_options(field_name)
                    field_config["required"] = True
    ...
    def _generate_validation_rules(self, fields: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field_name, field_config in fields.items():
            ...
            if field_config.get("max_length"):
                field_validations.append(f"max_length:{field_config['max_length']}")
            
            if field_config.get("min_value") is not None:
                field_validations.append(f"min_value:{field_config['min_value']}")
            
            if field_config.get("max_value") is not None:
                field_validations.append(f"max_value:{field_config['max_value']}")
    ...
    def _generate_form_validation_rules(self, fields: Dict[str, Any]) -> Dict[str, Any]:
        """Generate validation rules for form fields."""
        return self._generate_validation_rules(fields)  # Same logic for now
    
    def _generate_ui_config(self, fields: Dict[str, Any], table_name: str) -> Dict[str, Any]:
        ...
        for field_name, field_config in fields.items():
            ...
            if field_type in ["string", "integer", "date", "boolean", "select"] and len(list_columns) < 5:
            ...
            if field_type in ["string", "integer", "float", "date", "datetime", "boolean"]:
        ...
        return {
            ...
            "form_view": {
                ...
                "hidden_fields": [name for name in field_names if name.endswith("_at")]
            }
        }
    
    def _generate_form_layout(self, fields: Dict[str, Any], form_name: str) -> Dict[str, Any]:
        ...
        return {
            "sections": sections,
            "field_groups": {
                "basic": [name for name in field_names if not name.endswith("_at")],
                "timestamps": [name for name in field_names if name.endswith("_at")]
            }
        }
    
    def _generate_validation_notes(self, config: Dict[str, Any], requirements: str) -> List[str]:
        """Generate validation and usage notes for the configuration."""
        notes = [
            "This configuration follows GenericSuite patterns and conventions.",
            "Ensure all required fields are properly validated in your application.",
            "Review field types and constraints based on your specific requirements.",
            "Test the configuration in a development environment before production use."
        ]
        
        # Add specific notes based on configuration content
        if "table_config" in config:
            notes.append("Table configuration includes CRUD permissions and UI settings.")
            if config["table_config"].get("relationships"):
                notes.append("Review relationship configurations for proper foreign key constraints.")
        
        if "form_config" in config:
            notes.append("Form configuration includes validation rules and UI layout.")
            notes.append("Customize the submit endpoint and success/error messages as needed.")
        
        return notes
    
    def _generate_config_examples(self, config: Dict[str, Any], config_type: str) -> Dict[str, Any]:
    ...
    def validate_configuration(self, config: Dict[str, Any], config_type: str) -> Tuple[bool, List[str]]:
        """
    ...
    def _validate_table_config(self, config: Dict[str, Any]) -> List[str]:
        ...
        if "fields" not in table_config:
            ...
            if not isinstance(fields, dict) or not fields:
                errors.append("table_config.fields must be a non-empty dictionary")
    ...
    def _validate_form_config(self, config: Dict[str, Any]) -> List[str]:
        ...
        if "fields" not in form_config:
            ...
            if not isinstance(fields, dict) or not fields:
                errors.append("form_config.fields must be a non-empty dictionary")
...
def create_knowledge_base_search_tool() -> Tool:
    """
    ...
    def search_knowledge_base(query: KnowledgeBaseQuery) -> KnowledgeBaseSearchResults:
        """
    ...
    return Tool(search_knowledge_base, description=(
        ...
        "context before generating code or answering questions about GenericSuite."
    ))


def create_context_retrieval_tool() -> Tool:
    """
    Create a Pydantic AI tool for context retrieval optimized for code generation.
    ...
    class ContextQuery(BaseModel):
        ...
        max_length: int = Field(default=4000, description="Maximum context length in characters")
        file_type: Optional[str] = Field(default=None, description="Optional file type filter")
...
def create_json_config_generation_tool() -> Tool:
    """
    ...
    """
    json_generator = JSONConfigGenerator()
    
    def generate_json_configuration(request: JSONConfigRequest) -> JSONConfigResult:
        """
        Generate GenericSuite JSON configurations for tables, forms, and other components.
        
        This tool creates valid JSON configurations following GenericSuite patterns,
        including proper field definitions, validation rules, and UI configurations.
        
        Args:
            request: Configuration generation request with type and requirements.
            
        Returns:
            JSONConfigResult: Generated configuration with validation notes and examples.
        """
        if request.config_type == "table":
            ...
            config_name = request.table_name or f"generated_{request.config_type}"
    ...
    return Tool(generate_json_configuration, description=(
        ...
        "definitions, validation rules, and UI settings following GenericSuite patterns."
    ))


def create_config_validation_tool() -> Tool:
    """
    ...
    class ValidationRequest(BaseModel):
        ...
        configuration: Dict[str, Any] = Field(description="JSON configuration to validate")
        config_type: str = Field(description="Type of configuration (table, form, etc.)")
    ...
    def validate_json_configuration(request: ValidationRequest) -> ValidationResult:
        """
        Validate GenericSuite JSON configurations against patterns and requirements.
        ...
        for error in errors:
            if "missing" in error.lower():
                suggestions.append("Add the missing required fields to complete the configuration")
            elif "type" in error.lower():
                suggestions.append("Ensure all fields have valid type definitions")
            elif "empty" in error.lower():
                suggestions.append("Provide at least one field definition in the configuration")
        
        if not suggestions and is_valid:
            suggestions.append("Configuration looks good! Consider adding more detailed validation rules.")
...
def get_all_knowledge_base_tools() -> List[Tool]:
    """
    ...
    """
    return [
        create_knowledge_base_search_tool(),
        create_context_retrieval_tool()
    ]
...
class PythonCodeRequest(BaseModel):
    ...
    code_type: str = Field(description="Type of code (langchain_tool, mcp_tool, utility, api_endpoint)")
    requirements: str = Field(description="Requirements and specifications for the code")
    tool_name: str = Field(description="Name of the tool or function to generate")
    include_tests: bool = Field(default=True, description="Include unit tests")
    include_docs: bool = Field(default=True, description="Include comprehensive documentation")
    framework: Optional[str] = Field(default=None, description="Specific framework (fastmcp, langchain)")
...
class PythonCodeResult(BaseModel):
    ...
    usage_example: str = Field(description="Example usage of the generated code")
    test_code: Optional[str] = Field(default=None, description="Unit test code")
...
class {tool_class_name}(BaseTool):
    ...
    def _run(self, {run_parameters}) -> str:
        ...
        """
        try:
            logger.info(f"Executing {tool_name} with parameters: {{{run_parameters}}}")
...
def {function_name}({function_parameters}) -> {return_type}:
    ...
    """
    try:
        logger.debug(f"Executing {function_name} with parameters: {{{function_parameters}}}")
...
'''
    ...
    def generate_langchain_tool(self, requirements: str, tool_name: str) -> PythonCodeResult:
        """
        ...
        try:
            ...
            implementation_code = self._generate_implementation_code(requirements, "langchain")
            ...
            code = self.langchain_tool_template.format(
                ...
                tool_description=f"GenericSuite Langchain tool that {requirements}",
            ...
            usage_example = self._generate_langchain_usage_example(tool_class_name, tool_name)
            
            # Generate test code
            test_code = self._generate_test_code(tool_name, "langchain") if True else None
            
            # Generate documentation
            documentation = self._generate_documentation(tool_name, "langchain", requirements)
    ...
    def generate_mcp_tool(self, requirements: str, tool_name: str) -> PythonCodeResult:
        """
        ...
        try:
            ...
            # Generate implementation code
            implementation_code = self._generate_implementation_code(requirements, "mcp")
            
            # Generate function parameters
            function_parameters = self._generate_function_parameters(args_fields)
            ...
            code = self.mcp_tool_template.format(
                ...
                function_args_docs=self._generate_function_args_docs(args_fields),
            ...
            # Generate usage example
            usage_example = self._generate_mcp_usage_example(tool_function_name, tool_name)
            
            # Generate test code
            test_code = self._generate_test_code(tool_name, "mcp") if True else None
            
            # Generate documentation
            documentation = self._generate_documentation(tool_name, "mcp", requirements)
    ...
    def generate_utility_function(self, requirements: str, function_name: str) -> PythonCodeResult:
        """
        ...
        try:
            ...
            function_parameters = self._generate_utility_parameters(requirements)
            return_type = self._determine_return_type(requirements)
            
            # Generate implementation code
            implementation_code = self._generate_implementation_code(requirements, "utility")
            ...
            code = self.utility_template.format(
                ...
                function_description=f"GenericSuite utility function that {requirements}",
                function_args_docs=self._generate_utility_args_docs(function_parameters),
            ...
            # Generate test code
            test_code = self._generate_test_code(function_name, "utility") if True else None
            
            # Generate documentation
            documentation = self._generate_documentation(function_name, "utility", requirements)
    ...
    def _generate_input_fields(self, requirements: str) -> str:
        ...
        if "query" in requirements.lower() or "search" in requirements.lower():
            fields.append('query: str = Field(description="Search query or input text")')
        
        if "limit" in requirements.lower() or "count" in requirements.lower():
            fields.append('limit: int = Field(default=10, description="Maximum number of results")')
        
        if "filter" in requirements.lower():
            fields.append('filter_type: Optional[str] = Field(default=None, description="Optional filter criteria")')
        
        if not fields:
            fields.append('input_data: str = Field(description="Input data for processing")')
    ...
    def _generate_implementation_code(self, requirements: str, code_type: str) -> str:
    ...
    def _generate_utility_parameters(self, requirements: str) -> str:
        ...
        if "data" in requirements.lower():
            return "data: Dict[str, Any], options: Optional[Dict[str, Any]] = None"
    ...
    def _determine_return_type(self, requirements: str) -> str:
        ...
        elif "dict" in requirements.lower() or "object" in requirements.lower():
            return "Dict[str, Any]"
...
'''
    ...
    def _generate_langchain_usage_example(self, class_name: str, tool_name: str) -> str:
...
'''
    ...
    def _generate_mcp_usage_example(self, function_name: str, tool_name: str) -> str:
...
'''
    ...
    def _generate_documentation(self, name: str, code_type: str, requirements: str) -> str:
...
def get_all_json_generation_tools() -> List[Tool]:
    """
    ...
    """
    return [
        create_json_config_generation_tool(),
        create_config_validation_tool()
    ]


def create_python_code_generation_tool() -> Tool:
    """
    ...
    def generate_python_code(request: PythonCodeRequest) -> PythonCodeResult:
        """
        Generate Python code for GenericSuite applications including Langchain Tools and MCP Tools.
        
        This tool creates well-structured Python code following GenericSuite patterns,
        including proper error handling, documentation, and test code.
        
        Args:
            request: Code generation request with type, requirements, and options.
...
class FrontendCodeRequest(BaseModel):
    ...
    component_type: str = Field(description="Type of component (form, table, page, component)")
    requirements: str = Field(description="Requirements and specifications for the frontend code")
    component_name: str = Field(description="Name of the component to generate")
    include_styling: bool = Field(default=True, description="Include CSS/styling")
    include_tests: bool = Field(default=True, description="Include component tests")
    ui_framework: str = Field(default="react", description="UI framework (react, vue, angular)")


class BackendCodeRequest(BaseModel):
    """Request model for backend code generation."""
    framework: str = Field(description="Backend framework (fastapi, flask, chalice)")
    code_type: str = Field(description="Type of code (api_endpoint, model, service, middleware)")
    requirements: str = Field(description="Requirements and specifications for the backend code")
    module_name: str = Field(description="Name of the module/endpoint to generate")
    include_auth: bool = Field(default=True, description="Include authentication")
    include_validation: bool = Field(default=True, description="Include input validation")
    include_tests: bool = Field(default=True, description="Include unit tests")
...
class CodeGenerationResult(BaseModel):
    ...
    files: Dict[str, str] = Field(description="Additional files (tests, styles, etc.)")
    imports: List[str] = Field(description="Required imports and dependencies")
    usage_instructions: str = Field(description="Instructions for using the generated code")
    integration_notes: str = Field(description="Notes for integrating with GenericSuite")
    sources: List[str] = Field(default_factory=list, description="Source documents used")
...
class FrontendCodeGenerator:
    ...
    def _load_templates(self) -> None:
        ...
        self.react_component_template = '''import React, {{ useState, useEffect }} from 'react';
...
'''
        ...
        self.react_form_template = '''import React, {{ useState }} from 'react';
...
'''
    ...
    def generate_react_component(self, requirements: str, component_name: str,
                                component_type: str = "component") -> CodeGenerationResult:
        """
        ...
        try:
            ...
            context, sources = self.kb_tool.get_context_for_generation(
                query=f"GenericSuite React component {requirements} {component_type}",
                max_context_length=3000,
                file_type_filter="jsx"
            )
            
            if component_type == "form":
                return self._generate_form_component(requirements, component_name, sources)
            else:
                return self._generate_generic_component(requirements, component_name, component_type, sources)
    ...
    def _generate_form_component(self, requirements: str, component_name: str,
        ...
        files = {
            f"{component_name}.test.tsx": self._generate_component_test(component_name, "form"),
            f"{component_name}.stories.tsx": self._generate_storybook_story(component_name, "form")
        }
        ...
        return CodeGenerationResult(
            ...
            usage_instructions=self._generate_form_usage_instructions(component_name),
            integration_notes=self._generate_integration_notes("react", "form"),
            sources=sources
        )
    
    def _generate_generic_component(self, requirements: str, component_name: str,
        ...
        component_jsx = self._generate_component_jsx(requirements, component_type)
        ...
        code = self.react_component_template.format(
            ...
            props_destructuring=self._generate_props_destructuring(props_interface),
        ...
        # Generate additional files
        files = {
            f"{component_name}.test.tsx": self._generate_component_test(component_name, component_type),
            f"{component_name}.module.css": self._generate_component_styles(component_name, component_type)
        }
        ...
        return CodeGenerationResult(
            ...
            usage_instructions=self._generate_component_usage_instructions(component_name, component_type),
            integration_notes=self._generate_integration_notes("react", component_type),
            sources=sources
        )
    ...
    def _parse_form_fields(self, requirements: str) -> List[Dict[str, Any]]:
        ...
        field_patterns = {
            ...
            "status": {"type": "select", "required": False, "options": ["active", "inactive"]},
            "category": {"type": "select", "required": False, "options": ["general", "important"]}
        }
    ...
    def _generate_validation_schema(self, fields: List[Dict[str, Any]]) -> str:
        ...
        for field in fields:
            ...
            if field_type == "email":
                ...
                validation = "z.string().min(10, 'Must be at least 10 characters')"
            else:
    ...
    def _generate_component_jsx(self, requirements: str, component_type: str) -> str:
        """Generate component JSX."""
        return f"{{/* {component_type} component JSX based on requirements */}}"
    ...
    def _generate_component_test(self, component_name: str, component_type: str) -> str:
...
'''
    ...
    def _generate_storybook_story(self, component_name: str, component_type: str) -> str:
...
'''
    ...
    def _generate_component_styles(self, component_name: str, component_type: str) -> str:
...
'''
    ...
    def _generate_component_usage_instructions(self, component_name: str, component_type: str) -> str:
...
'''
    ...
    def _generate_integration_notes(self, framework: str, component_type: str) -> str:
...
{endpoint_name}_bp = Blueprint('{endpoint_name}', __name__, url_prefix='/{endpoint_prefix}')
...
'''
    ...
    def generate_backend_code(self, requirements: str, module_name: str,
                            framework: str, code_type: str = "api_endpoint") -> CodeGenerationResult:
        """
        ...
        try:
            ...
            if framework == "fastapi":
                return self._generate_fastapi_code(requirements, module_name, code_type, sources)
            elif framework == "flask":
                return self._generate_flask_code(requirements, module_name, code_type, sources)
            elif framework == "chalice":
                return self._generate_chalice_code(requirements, module_name, code_type, sources)
            else:
    ...
    def _generate_fastapi_code(self, requirements: str, module_name: str,
                             code_type: str, sources: List[str]) -> CodeGenerationResult:
        """Generate FastAPI code."""
        # Generate code components
        endpoints = self._generate_fastapi_endpoints(requirements, module_name)
        request_response_models = self._generate_pydantic_models(requirements, module_name)
        helper_functions = self._generate_helper_functions(requirements, "fastapi")
        ...
        # Generate additional files
        files = {
            f"test_{module_name}.py": self._generate_backend_test(module_name, "fastapi"),
            f"{module_name}_models.py": self._generate_models_file(module_name, "fastapi"),
            f"{module_name}_schemas.py": self._generate_schemas_file(module_name, "fastapi")
        }
        ...
        return CodeGenerationResult(
            ...
            usage_instructions=self._generate_backend_usage_instructions(module_name, "fastapi"),
            integration_notes=self._generate_backend_integration_notes("fastapi"),
            sources=sources
        )
    
    def _generate_flask_code(self, requirements: str, module_name: str,
                           code_type: str, sources: List[str]) -> CodeGenerationResult:
        """Generate Flask code."""
        # Generate code components
        ...
        helper_functions = self._generate_helper_functions(requirements, "flask")
        ...
        # Generate additional files
        files = {
            f"test_{module_name}.py": self._generate_backend_test(module_name, "flask"),
            f"{module_name}_models.py": self._generate_models_file(module_name, "flask")
        }
        ...
        return CodeGenerationResult(
            ...
            usage_instructions=self._generate_backend_usage_instructions(module_name, "flask"),
            integration_notes=self._generate_backend_integration_notes("flask"),
            sources=sources
        )
    
    def _generate_chalice_code(self, requirements: str, module_name: str,
                             code_type: str, sources: List[str]) -> CodeGenerationResult:
        """Generate Chalice code."""
        # Generate code components
        routes = self._generate_chalice_routes(requirements, module_name)
        validation_schemas = self._generate_chalice_schemas(requirements, module_name)
        helper_functions = self._generate_helper_functions(requirements, "chalice")
        ...
        # Generate additional files
        files = {
            f"test_{module_name}.py": self._generate_backend_test(module_name, "chalice"),
            f"{module_name}_models.py": self._generate_models_file(module_name, "chalice")
        }
        ...
        return CodeGenerationResult(
            ...
            usage_instructions=self._generate_backend_usage_instructions(module_name, "chalice"),
            integration_notes=self._generate_backend_integration_notes("chalice"),
            sources=sources
        )
    
    # Helper methods for backend code generation
    def _generate_fastapi_endpoints(self, requirements: str, module_name: str) -> str:
...
async def delete_{module_name}(
    ...
    def _generate_flask_routes(self, requirements: str, module_name: str) -> str:
...
def get_{module_name}(item_id):
    ...
    def _generate_chalice_routes(self, requirements: str, module_name: str) -> str:
...
def create_{module_name}():
    ...
    try:
        ...
        # Validate and process data
        return format_response({{"id": 1, "message": "Created successfully"}}, 201)
    except Exception as e:
        logger.error(f"Error creating {module_name}: {{e}}")
        return Response(
            body=json.dumps({{"error": "Internal server error"}}),
    ...
    # More helper methods would continue here...
    def _generate_pydantic_models(self, requirements: str, module_name: str) -> str:
...
class {module_name.title()}Response({module_name.title()}Base):
    ...
    def _generate_marshmallow_schemas(self, requirements: str, module_name: str) -> str:
...
{module_name}_update_schema = {module_name.title()}UpdateSchema()'''
    ...
    def _generate_chalice_schemas(self, requirements: str, module_name: str) -> str:
...
def validate_{module_name}_update(data):
    ...
    def _generate_helper_functions(self, requirements: str, framework: str) -> str:
        """Generate helper functions."""
        return f'''def format_response(data, status_code=200):
    """Format API response."""
    return {{
        "data": data,
        "status": "success",
...
'''
    ...
    def _generate_backend_usage_instructions(self, module_name: str, framework: str) -> str:
...
'''
    ...
    def _get_registration_instructions(self, framework: str, module_name: str) -> str:
...
def get_all_python_generation_tools() -> List[Tool]:
    """
    ...
    """
    return [
        create_python_code_generation_tool()
    ]


def create_frontend_code_generation_tool() -> Tool:
    """
    ...
    def generate_frontend_code(request: FrontendCodeRequest) -> CodeGenerationResult:
        """
        ...
        Args:
            request: Frontend code generation request with component specifications.
            
        Returns:
            CodeGenerationResult: Generated frontend code with tests and styles.
        """
    ...
    return Tool(generate_frontend_code, description=(
        ...
        "Creates modern, responsive components with TypeScript, proper styling, "
        "and integration with GenericSuite patterns and APIs."
    ))


def create_backend_code_generation_tool() -> Tool:
    """
    ...
    def generate_backend_code(request: BackendCodeRequest) -> CodeGenerationResult:
        """
        ...
        Args:
            request: Backend code generation request with framework and specifications.
            
        Returns:
            CodeGenerationResult: Generated backend code with tests and documentation.
        """
...
def get_all_frontend_backend_tools() -> List[Tool]:
    """
    ...
    """
    return [
        create_frontend_code_generation_tool(),
        create_backend_code_generation_tool()
    ]
...
def get_all_agent_tools() -> List[Tool]:
    ...
    """
    tools = []
    tools.extend(get_all_knowledge_base_tools())
    tools.extend(get_all_json_generation_tools())
    tools.extend(get_all_python_generation_tools())
    tools.extend(get_all_frontend_backend_tools())
    return tools
...
if __name__ == "__main__":
    ...
    async def test_knowledge_base_tool():
        ...
        try:
            ...
            # Test search
            results = kb_tool.search("GenericSuite table configuration", limit=3)
