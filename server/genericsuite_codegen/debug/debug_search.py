...
async def test_knowledge_base_tool():
    ...
    try:
        ...
        print(
            f"Knowledge base tool found {len(search_results.results)}"
            " results:")
...
async def test_sample_documents():
    ...
    try:
        ...
        for i, doc in enumerate(sample_docs):
            ...
            print(
                f"      Embedding dimension: {len(doc.get('embedding', []))}")
...
async def main():
    ...
    search_works = await test_vector_search(db_manager, embedding_gen,
                                            query_embedding)
    if not search_works:
