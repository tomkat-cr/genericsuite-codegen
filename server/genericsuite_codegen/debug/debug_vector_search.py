...
from genericsuite_codegen.document_processing.embeddings \
    import create_embedding_generator
...
def test_vector_search_detailed():
    ...
    try:
        ...
        for i, doc in enumerate(sample_docs):
            ...
            print(
                "    Embedding sample: "
                + f"{embedding[:5] if embedding else 'None'}")
        ...
        try:
            ...
            print("This is expected if MongoDB Atlas vector search"
                  " is not available")
        ...
        try:
                        ...
                        "similarity": {
                            "$let": {
                                "vars": {
                                    "dot_product": {
                                        "$reduce": {
                                            "input": {"$range": [0, {"$size": "$embedding"}]},  # noqa: E501
                                            "initialValue": 0,
                                            "in": {
                                                "$add": [
                                                    "$$value",
                                                    {
                                                        "$multiply": [
                                                            {"$arrayElemAt": [
                                                                "$embedding", "$$this"]},  # noqa: E501
                                                            {"$arrayElemAt": [
                                                                query_embedding, "$$this"]}  # noqa: E501
                                                        ]
                                                    }
                                                ]
                                            }
                                ...
                                "in": {
                                    "$divide": [
                                        "$$dot_product",
                                        {"$multiply": [
                                            "$$query_magnitude", "$$doc_magnitude"]}  # noqa: E501
                                    ]
                                }
                            }
                        }
            ...
            for i, result in enumerate(cosine_results):
                print(
                    f"{i+1}. Similarity: "
                    f"{result.get('similarity', 'N/A'):.4f}")
                print(f"      Path: {result.get('path', 'unknown')}")
                print(f"      Content: {result.get('content', '')[:100]}...")
                print()
