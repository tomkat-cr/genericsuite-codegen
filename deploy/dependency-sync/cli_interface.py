...
class CLIInterface:
    ...
    This class handles all command-line interactions including argument parsing,
    file validation, progress reporting, and error handling. It provides a
    consistent interface for both interactive and automated usage.
    ...
    def _create_parser(self) -> argparse.ArgumentParser:
        ...
        parser = argparse.ArgumentParser(
            description="Synchronize Poetry dependencies from pyproject.toml files to Dockerfile pip install commands",
            formatter_class=argparse.RawDescriptionHelpFormatter,
            epilog="""
Examples:
  %(prog)s server/pyproject.toml deploy/docker_images/Dockerfile
  %(prog)s server/pyproject.toml mcp-server/pyproject.toml deploy/docker_images/Dockerfile
  %(prog)s --verbose --dry-run server/pyproject.toml deploy/docker_images/Dockerfile
            """
        )
    ...
    def parse_arguments(self, args: Optional[List[str]] = None) -> argparse.Namespace:
        ...
        try:
            ...
            if parsed_args.verbose and parsed_args.quiet:
                self.parser.error("--verbose and --quiet options are mutually exclusive")
    ...
    def validate_file_paths(self, source_files: List[str], target_dockerfile: str) -> None:
        """
        ...
        for source_file in source_files:
            ...
            if not os.access(source_path, os.R_OK):
                raise PermissionError(f"Cannot read source file: {source_file}")
            
            # Validate it's a pyproject.toml file
            if source_path.name != 'pyproject.toml':
                raise ValueError(f"Source file must be named 'pyproject.toml': {source_file}")
        
        # Validate target dockerfile
        dockerfile_path = Path(target_dockerfile)
        
        if not dockerfile_path.exists():
            raise FileNotFoundError(f"Target Dockerfile not found: {target_dockerfile}")
        
        if not dockerfile_path.is_file():
            raise ValueError(f"Target path is not a file: {target_dockerfile}")
        
        if not os.access(dockerfile_path, os.R_OK):
            raise PermissionError(f"Cannot read target Dockerfile: {target_dockerfile}")
        
        if not os.access(dockerfile_path, os.W_OK):
            raise PermissionError(f"Cannot write to target Dockerfile: {target_dockerfile}")
    
    def print_summary(self, changes: Dict[str, Any], verbose: bool = False, quiet: bool = False) -> None:
        """
    ...
    def print_progress(self, message: str, verbose: bool = False, quiet: bool = False) -> None:
        """
