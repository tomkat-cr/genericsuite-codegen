...
class DockerfileUpdater:
    ...
    def _write_dockerfile(self, dockerfile_path: str, content: str) -> None:
        ...
        if not os.access(os.path.dirname(dockerfile_path) or '.', os.W_OK):
            raise DockerfileUpdateError(
                f"Cannot write to directory: {os.path.dirname(dockerfile_path)}",
                dockerfile_path=dockerfile_path
            )
    ...
    def find_pip_install_block(self, content: str) -> Tuple[int, int]:
        ...
        if not start_match:
            raise DockerfileUpdateError(
                ...
                "Expected format: RUN pip install --upgrade pip && pip install --no-cache-dir ..."
            )
        
        start_idx = start_match.start()
        
        # Find the end of the pip install block by looking for the next RUN, COPY, EXPOSE, CMD, etc.
        # or end of file
        remaining_content = content[start_match.end():]
        
        # Pattern to match the end of the pip install block
        # This matches either:
        # 1. A line that doesn't start with whitespace and contains a package (last dependency line)
            ...
            if in_pip_block:
                ...
                if (stripped_line.startswith('"') and stripped_line.endswith('"')) or \
                   (stripped_line.startswith('"') and stripped_line.endswith('" \\')):
                    ...
                    # If this line doesn't end with backslash, it's the last dependency
                    if not stripped_line.endswith(' \\'):
                        end_offset += 0  # Don't include the newline after the last dependency
                        break
        ...
        # If we didn't find a clear end, look for the next Dockerfile instruction
        if in_pip_block:
    ...
    def format_pip_dependencies(self, dependencies: Dict[str, str]) -> str:
        ...
        for name, constraint in sorted(dependencies.items()):
            ...
            if constraint and constraint.strip() and constraint != "*":
                # Clean up constraint (remove extra whitespace and normalize spaces)
