...
to pip-compatible version specifications. It supports all major Poetry constraint
types including caret (^), tilde (~), exact (==), and range constraints.

Poetry uses different constraint syntax than pip, so this module bridges that gap:
...
class VersionTranslator:
    ...
    def handle_caret_constraint(self, version: str) -> str:
        ...
        if not self._is_valid_version(version):
            raise VersionConstraintError(f"Invalid version in caret constraint: {version}")
    ...
    def handle_tilde_constraint(self, version: str) -> str:
        ...
        """
        if not self._is_valid_version(version):
            raise VersionConstraintError(f"Invalid version in tilde constraint: {version}")
    ...
    def handle_extras(self, dependency: str, constraint: str) -> str:
        ...
        Example:
            >>> translator = VersionTranslator()
            >>> result = translator.handle_extras('uvicorn[standard]', '^0.24.0')
            >>> print(result)
            'uvicorn[standard]>=0.24.0,<0.25.0'
            >>> 
    ...
    def _is_valid_version(self, version: str) -> bool:
        ...
        """
        import re
        # More strict version pattern: major[.minor[.patch]][pre-release][+build]
        # Allows 1, 1.2, 1.2.3, 1.2.3a1, 1.2.3-beta.1, etc.
        # But not excessive dots like 1.2.3.4.5.6
        version_pattern = r'^\d+(\.\d+){0,2}([a-zA-Z]\d*|[-+][a-zA-Z0-9\.\-]*)?$'
        return bool(re.match(version_pattern, version))
    
    def translate_dependency_with_constraint(self, dependency_name: str, constraint: str) -> str:
        """
        Translate a full dependency specification with name and constraint.
        
        Args:
            dependency_name: Name of the dependency (may include extras like 'uvicorn[standard]')
