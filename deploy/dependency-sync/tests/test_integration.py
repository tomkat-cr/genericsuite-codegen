...
"""
    ...
    def test_no_backup_option(self, sample_pyproject_toml, sample_dockerfile):
        ...
        backup_files = [f for f in os.listdir(dockerfile_dir) if f.startswith("Dockerfile.backup")]
...
"""
        ...
        assert "pydantic>=2.0.0,<3.0.0" in content  # Should use the more restrictive ^2.0.0
    ...
    def test_different_dockerfile_formats(self, temp_dir, fixtures_dir):
        ...
        shutil.copy(multiline_dockerfile, os.path.join(temp_dir, "multiline_dockerfile"))
        ...
        shutil.copy(single_line_dockerfile, os.path.join(temp_dir, "single_line_dockerfile"))
...
"""
        ...
        dependencies = extract_pip_dependencies_from_dockerfile(dockerfile_path)
        
        # Verify format is correct
        for dep in dependencies:
            # Should be quoted strings with proper version constraints
            assert not dep.startswith('"'), f"Dependency should not have quotes: {dep}"
            assert not dep.endswith('"'), f"Dependency should not have quotes: {dep}"
